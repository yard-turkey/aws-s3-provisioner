## AWS S3 Provisioner - Access Existing Bucket with OBC Example
This example will walk through the basic steps needed to grant access and use 
an existing AWS S3 bucket giving the requester a new user, policy and access id and keys 
...
parameters:
  ...
  bucketName: my-existing-bucket [6]
...
1. Namespace where the Secret will exist.
1. (optional) If this exists, it means that the bucket already exists and the user is requesting access to 
the specific bucket listed. If this is blank or not included in the StorageClass parameters - it indicates that a *NEW* bucket will be
...
**NOTE:** In this example, we exclude the name of the bucket or any reference to it, all the user needs
to know is what StorageClass they need to reference, the Administrator creates the StorageClass to handle
the rest, including name of bucket and any access restrictions that need to be placed on the existing bucket.
...
relationship. In that OB, it will reference the user that
was created, storageclass and endpoint information for the bucket.
...
spec:
  ...
  claimRef: null
  reclaimPolicy: null
  storageClassName: s3-existing-buckets [4]
```
...
data:
  ...
  BUCKET_SSL: "true"
  BUCKET_SUBREGION: ""
  BUCKET_URL: s3-us-west-1.amazonaws.com:443/us-west-1/my-existing-bucket [3]
kind: ConfigMap
metadata:
...
spec:
  ...
  - env:
    - name: BUCKET_NAME
      value: "my-existing-bucket" [1]
    - name: BUCKET_ID [2]
      valueFrom:
            secretKeyRef:
              name: my-existing-bucket [4]
